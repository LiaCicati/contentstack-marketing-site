...
function resolveHref(
  ...
  if (item.page_reference && item.page_reference.length > 0) {
    ...
    if (ref.url) {
      const pagePath = ref.url === "/" ? "" : ref.url;
...
export default function Navigation({ navigation, siteName, logo, locale }: Props) {
          ...
          <ul className="flex items-center gap-1">
            {navigation.nav_items.map((item) => {
              ...
              return (
                ...
                >
                  <Link
                    href={resolveHref(item, localePrefix)}
                    {...(isExternal(item) && {
                      target: "_blank",
                    ...
                    <div className="absolute left-0 top-full w-52 pt-2">
                      <ul className="rounded-lg bg-white py-2 shadow-lg ring-1 ring-gray-200">
                        {item.children!.map((child) => (
                          <li key={child.label}>
                            <Link
                              href={resolveHref(child, localePrefix)}
                              {...(isExternal(child) && {
                                target: "_blank",
                                rel: "noopener noreferrer",
                              })}
